...
datalakeSAPCurr = async(sapinfo,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
        async function OrchestrationDatalakeSAPCurr(callback){//SAP
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            sapstream =  fs.createWriteStream("./blob/"+foldername+"/Datalake_SAP_Variable.tf")
            ...
            sapstream.write('\nvariable "DataLakeStorageAccountName" { \n type = "string"\n default = '+JSON.stringify(sapinfo.Staging.datalakeName)+'\n}')
            sapstream.write('\nvariable "DataLakeStoaregConnectionString" { \n type = "string"\n default = '+JSON.stringify(sapinfo.Staging.datalakeAccessKey)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/Datalake_SAP_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/Datalake_SAP_Variable.tf")
        })
        }catch(error){
        callback(error)
        }
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await datalakeSAPCurr(sapinfo,cred,foldername).then((res)=>{
                    console.log("res",res)
...
module.exports = {datalakeSAPCurr}
