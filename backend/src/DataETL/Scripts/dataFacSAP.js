...
sapCurr = async(sapinfo,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
            async function OrchestrationSapCurr(callback){//SAP
                try{
                await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
                sapstream = fs.createWriteStream("./blob/"+foldername+"/SAP_Variable.tf")
                ...
                sapstream.write('\nvariable "SQLConnectionString" { \n type = "string"\n default = '+JSON.stringify(sapinfo.Staging.sqlConnStr)+'\n}')
                ...
                callback(null,'./src/DataETL/TerraformResourceFiles/SAP_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/SAP_Variable.tf")
                })
                }catch(error){
                    callback(error)
                }
            ...
            async function sendtoBlob(fileName,callback){
                ...
                if(err){
                    await sapCurr(sapinfo,cred,foldername).then((res)=>{
                        console.log("res",res)
