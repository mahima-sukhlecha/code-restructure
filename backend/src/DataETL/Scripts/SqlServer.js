...
sqlserverCurr = async(sqlinfo,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
        async function OrchestrationSqlserverCurr(callback){ // foldername -- Storage_DB
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            sqlstream = fs.createWriteStream("./blob/"+foldername+"/SQLServer_Variable.tf")
            ...
            sqlstream.write('\nvariable "SQLConnectionString" { \n type = "string"\n default = '+JSON.stringify(sqlinfo.Staging.sqlConnStr)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/SQLServer_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/SQLServer_Variable.tf")  
        })
        }catch(error){
            callback(error)
        }
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await sqlserverCurr(sqlinfo,cred,foldername).then((res)=>{
                    console.log("res",res)
