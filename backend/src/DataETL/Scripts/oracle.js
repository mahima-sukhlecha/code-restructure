...
oracleCurr = async(orinfo,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
        async function OrchestrationOracleCurr(callback){ // foldername -- Storage_DB
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            orstream =  fs.createWriteStream("./blob/"+foldername+"/Oracle_Variable.tf")
            ...
            orstream.write('\nvariable "SQLConnectionString" { \n type = "string"\n default = '+JSON.stringify(orinfo.Staging.sqlConnStr)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/Oracle_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/Oracle_Variable.tf")
                })
        }catch(error){
            callback(error)
        }
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await oracleCurr(orinfo,cred,foldername).then((res)=>{
                    console.log("res",res)
