...
datalakeOracleCurr = async(orinfo,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
        async function OrchestrationDatalakeOracleCurr(callback){// Oracle
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            orstream = fs.createWriteStream("./blob/"+foldername+"/Datalake_Oracle_Variable.tf")
            ...
            orstream.write('\nvariable "DataLakeStorageAccountName" { \n type = "string"\n default = '+JSON.stringify(orinfo.Staging.datalakeName)+'\n}')
        
            orstream.write('\nvariable "DataLakeStoaregConnectionString" { \n type = "string"\n default = '+JSON.stringify(orinfo.Staging.datalakeAccessKey)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/Datalake_Oracle_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/Datalake_Oracle_Variable.tf")
        })
        }catch(error){
            callback(error)
        } 
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await datalakeOracleCurr(orinfo,cred,foldername).then((res)=>{
                    console.log("res",res)
...
module.exports = {datalakeOracleCurr}