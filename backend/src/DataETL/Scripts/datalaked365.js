...
dynamic365_DLCurr = async(info,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
        async function OrchestrationDynamic365_DLCurr(callback){ // foldername -- Dynamic365
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            stream = fs.createWriteStream("./blob/"+foldername+"/Datalake_d365_Variable.tf")
            ...
            stream.write('\nvariable "DataLakeStorageAccountName" { \n type = "string"\n default = '+JSON.stringify(info.Staging.datalakeName)+'\n}')
        
            stream.write('\nvariable "DataLakeStoaregConnectionString" { \n type = "string"\n default = '+JSON.stringify(info.Staging.datalakeAccessKey)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/Datalake_d365_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/Datalake_d365_Variable.tf")
        })
        }catch(error){
           callback(error)
        }
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await dynamic365_DLCurr(info,cred,foldername).then((res)=>{
                    console.log("res",res)
 ...
 module.exports = {dynamic365_DLCurr}
