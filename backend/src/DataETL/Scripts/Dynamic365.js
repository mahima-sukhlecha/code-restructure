...
dynamic365Curr = async(info,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{
        waterfall([
        async function OrchestrationDynamic365Curr(callback){ // foldername -- Storage_DB
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            stream = fs.createWriteStream("./blob/"+foldername+"/Dynamic365_Variable.tf")
            ...
            stream.write('\nvariable "SQLConnectionString" { \n type = "string"\n default = '+JSON.stringify(info.Staging.sqlConnStr)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/Dynamic365_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/Dynamic365_Variable.tf") 
        })
        }catch(error){
            callback(error)
        }
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await dynamic365Curr(info,cred,foldername).then((res)=>{
                    console.log("res",res)
