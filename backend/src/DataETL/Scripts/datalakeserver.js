...
sqlserver_DLCurr = async(sqlinfo,cred,foldername)=>{
    try{
        return new Promise(async(resolve,reject)=>{ 
        waterfall([
        async function OrchestrationSqlserverCurr(callback){  //SQL_SERVER
            try{
            await fs.promises.mkdir("./blob/"+foldername, { recursive: true }).then(res=>{
            sqlstream = fs.createWriteStream("./blob/"+foldername+"/Datalake_SQL_Variable.tf")
            ...
            sqlstream.write('\nvariable "DataLakeStorageAccountName" { \n type = "string"\n default = '+JSON.stringify(sqlinfo.Staging.datalakeName)+'\n}')
            sqlstream.write('\nvariable "DataLakeStoaregConnectionString" { \n type = "string"\n default = '+JSON.stringify(sqlinfo.Staging.datalakeAccessKey)+'\n}')
            ...
            callback(null,'./src/DataETL/TerraformResourceFiles/Datalake_SQL_Main.tf','./blob/'+foldername,foldername,"./blob/"+foldername+"/Datalake_SQL_Variable.tf")
        })
        }catch(error){
            callback(error)
        }   
        ...
        async function sendtoBlob(fileName,callback){
            ...
            if(err){
                await sqlserver_DLCurr(sqlinfo,cred,foldername).then((res)=>{
                    console.log("res",res)
...
module.exports =  {sqlserver_DLCurr}
